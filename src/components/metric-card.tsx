    ...
    <Card className={cn("glass-card border-none overflow-hidden", className)}>
      <CardContent className="p-4 flex flex-col h-full justify-between overflow-hidden">
        <div className="flex items-center justify-between mb-2">
          <span className="text-[10px] font-bold uppercase tracking-wider text-muted-foreground truncate mr-2">{title}</span>
          <div className={cn(
        ...
        <div className="flex flex-col min-w-0">
          <div className="flex items-baseline gap-1 min-w-0 overflow-hidden">
            ...
            {unit && <span className="text-xs text-muted-foreground shrink-0 font-bold">{unit}</span>}
          </div>
          {description && (
            <p className="text-[9px] text-muted-foreground mt-1 truncate font-medium">
              {description}
            </p>
          )}
        </div>
